...
> 日常升级请打开仓库根目录的 `三步安装CompanyOps.md`，使用服务器上的 `更新CompanyOps.cmd`：自动获取官方仓库新版、在服务器构建和校验，再打开现有升级器，无需从开发电脑拷贝或手工解压 ZIP。首次安装或离线场景仍可使用离线包。后续章节用于工程排障和高级接入。
...
| 平台自身升级、卸载 | `更新CompanyOps.cmd` 串联服务器 Git 快进、构建、校验和 Setup 受控升级；Setup 提供目录切换及失败恢复；没有通用卸载入口 | 日常升级使用源码仓库更新入口并沿用原程序/数据目录，构建环境需预先准备；`Install-OpsPlatform.ps1` 仍只支持首次安装，目标主机仍须现场验证 |
