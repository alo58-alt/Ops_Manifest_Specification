...
> 日常升级以仓库根目录的 `三步安装CompanyOps.md` 为准：开发机编译和测试，发布工具自动传送预编译文件，服务器只校验和安装。发布目录中的 `更新CompanyOps.cmd` 打开升级器；SSH 运维可使用命令行预检与升级。服务器不运行 SDK、npm 或自动化测试。后续章节用于工程排障和高级接入。
...
| IIS / staticSite / 任务计划发布入口切换 | 尚无对应生产激活适配器；`Plan` 会失败关闭 | 继续使用项目受审更新流程，不得绕过 `Plan` |
| PM2 L3 现场验收 | typed 载荷、精确单项迁移、健康门禁与失败恢复已实现并通过隔离自动化，尚未在共享 PM2 daemon 完成现场 UAT | 首次生产使用前按 owner、三个组件和其他项目隔离性逐项验收，不得把隔离演练当成现场证据 |
...
| 平台自身升级、卸载 | 开发机构建后由发布工具自动传包；服务器 `更新CompanyOps.cmd` 校验并调用 Setup，也支持 SSH 命令行升级；没有通用卸载入口 | 沿用原程序/数据目录与授权，命令行升级绑定前后完整版本并要求现有 Session Agent 所属用户；具体操作见根目录安装入口，首次安装仍使用 Setup |
| 多主机集中控制 | 当前阶段明确不需要，不属于生产阻塞项 | 每台主机独立安装一个 Agent 和 Console |

**结论：** 普通运维当前可以安全完成“安装 CompanyOps、保持只读、接入声明、查看状态”。Windows Service（含 NSSM）、interactiveApp 与满足 typed 契约的 pm2Legacy 已接入同一发布事务，但项目安装、更新、回滚和启停仍必须先在试点主机完成对应现场验收，不能只凭自动化测试直接照搬到生产。
...
- .NET SDK 至少 `10.0.302`；`global.json` 优先选择 `10.0.3xx` 最新补丁，缺少该功能版本时允许同一 `10.0` 系列的更高稳定功能版本（例如 `10.0.400`），不跨主/次版本，也不接受预览版；
