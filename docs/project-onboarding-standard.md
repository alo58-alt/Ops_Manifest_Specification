...
只有当前任务包含该项目的 CompanyOps 接入时，才按目标等级阅读本规范并准备相应材料；普通代码或文档任务不自动接入其他项目。L1 材料任务：

1. 核对并维护项目根的 `ops\project-manifest.json`，已有有效声明不重建；
2. 维护对应 `ops\README.md`，记录实际运行事实和验收边界，不制造空文档；
3. 为每个运行组件提供可自动判断成功或失败的健康探针；
4. 对本次材料运行相应验收；失败先定位具体声明或契约，只修当前范围相关问题，不盲目循环、弱化校验或扩修其他项目。达到 L1 完成条件后停止；范围外缺口如实报告。
...
仅在当前用户明确要求接入该项目时，为其完成 CompanyOps L1 只读接入材料；已有有效材料优先复用：
...
4. 缺少健康能力时先核对已有入口；当前接入目标确实需要且任务范围覆盖时补最小无副作用接口或心跳，不扩修无关业务。仅审计或整理材料的任务报告缺口，不自动进入代码实现；
...
- [ ] 依赖顺序可形成 DAG；
- [ ] 无服务器绝对路径和 Secret 明文；
- [ ] 按本次实际改动完成验证：纯声明/文档检查 Schema、语义、引用和差异；业务或启动契约变化运行相应最小测试/构建，不为无关端或未变化快照重复构建；
